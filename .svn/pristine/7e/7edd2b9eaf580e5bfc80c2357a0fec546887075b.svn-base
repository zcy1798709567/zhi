		...
		value: function addData(year, month) {
			...
			for(var i = 0; i < 42; i++) {
				...
				if(writemonth > month) {
					...
					if($(daysElement[i]).find("span").length<=0){
                        $(daysElement[i]).append("<span id='day_"+writeday+"' class=\"date-tab\"></span>");
					}
				}
				...
				if(this._options.showLunarCalendar) {
               	...
               	//$(daysElement[i]).append("<span id='day_"+writeday+"' class=\"date-tab leave-span\">假</span>");
				//添加today样式
		...
		value: function updateEvent() {
			...
			daysElement.forEach(function(v, i) {
				...
				v.onclick = function() {
                    ...
                    var day = $(calendar.selectDay).children(":first").text();
                    getScheduleList(day);
