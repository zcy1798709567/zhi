...
oA.success = OA.object("showContent", function (data) {
    oA.arr = OA.array();
    Object.keys(data).splice(0, Object.keys(data).length - 1).forEach(function (key) {
    ...
    this.tbody = function () {
        ...
        for (let val in data.tbody) {
            object[num] = OA.array();
            object[num].push(num+1);
            for (let key in data.field) {
                if (Object.keys(data.tbody[val]).includes(data.field[key].name)) {
                    object[num].push(oa.decipher(data.field[key].type,data.tbody[val][data.field[key].name]));
                }
            }
...
$(function () {
    ...
    oA.ele = OA.ele(["#txtdate", "#txtdate2", "!header", "#content", "main", "radio",
...
oA.before = OA.object('load', function (pageNumber) {
    this.getAjaxData = function () {
        ...
        OA.axios(oA.url.tableform.tablefield, postData, 'get').done(function (data) {
            if (pageNumber) {
    ...
    this.pullDownAction = function () {
        new oA.before.load(0).getAjaxData(0);
    };
    this.start = function () {
