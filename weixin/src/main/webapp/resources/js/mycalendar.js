(function ($) {
	$.fn.weekCalendar = function (options, param) {
		...
		var day = d.getDay();
		var arr = new Array();
		var html = '<div class="week-container">\<div class="week_head"></div>\<div class="time">\<div class="time_header"><h3>一</h3><h3>二</h3><h3>三</h3><h3>四</h3><h3>五</h3><h3>六</h3><h3>日</h3></div>\<div class="time_con"></div>\</div></div>';
		$ele.append(html);
		showWeek();
		function week(dd){
...
//					console.log(LunarDate.GetLunarDay(arr[i].nian,parseInt(arr[i].yue+1),arr[i].ri));
					lili[i] = '<div class="time_td current_date selected time_taday"><p>' + arr[i].ri + '</p><p>'+LunarDate.GetLunarDay(arr[i].nian,parseInt(arr[i].yue+1),arr[i].ri)+'</p></div>';
					if(options.data){
					...
					lili[i] = '<div class="time_td"><p>' + arr[i].ri +'</p><p>'+LunarDate.GetLunarDay(arr[i].nian,parseInt(arr[i].yue+1),arr[i].ri)+'</p></div>';
					if(options.data){
						for (var j =  0; j < options.data.length; j++) {
							if(new Date(arr[i].nian,arr[i].yue,arr[i].ri).getTime() == new Date(options.data[j].date).getTime()){
...
//			$ele.find(".time_td").css({"height":widthTime,"line-height":20+"px"});
		}
